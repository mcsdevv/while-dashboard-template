...
export function WebhookLogDetail({ logId }: WebhookLogDetailProps) {
  ...
  const sourceLabel = log.source === "gcal" ? "Google Calendar" : log.source === "notion" ? "Notion" : "-";
  const hasChannelDetails = log.source === "gcal" && (log.channelId || log.resourceState || log.messageNumber !== undefined);
      ...
      <Card>
        ...
        <CardContent className="pt-0">
          <DetailRow
            ...
            value={
              log.eventTitle ? (
                <span className="break-words">{log.eventTitle}</span>
              ) : (
                "-"
              )
            }
          ...
          <DetailRow
            ...
            value={
              log.eventId ? (
                <span className="font-mono text-xs break-all">{log.eventId}</span>
              ) : (
                "-"
              )
            }
